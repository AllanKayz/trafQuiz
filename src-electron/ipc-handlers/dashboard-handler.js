...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    // BOLT: Shared date objects for index-friendly range queries
    const now = new Date();
    const todayStart = new Date(now.setHours(0, 0, 0, 0));
    const todayEnd = new Date(now.setHours(23, 59, 59, 999));

    if (role === "admin") {
      // BOLT: Parallelized administrative dashboard queries to reduce latency
      ...
      const [
        ...
        Instructor.count(),
        // BOLT: Using Op.between for index utilization on start_time
        Exam.count({
          where: {
            start_time: { [Op.between]: [todayStart, todayEnd] }
          }
        }),
        // BOLT: Optimized revenue query with range filter instead of strftime
        sequelize.query(
          `SELECT sum(amount) as total FROM payments
           WHERE type="income" AND payment_date >= ?`,
          {
            replacements: [startOfMonth.toISOString()],
            type: QueryTypes.SELECT
          }
        ),
        // BOLT: Pass rate query is already efficient but now runs in parallel
        sequelize.query(
      ...
      if (instructorId) {
        // BOLT: Parallelized instructor-specific queries
      ...
      if (studentId) {
        // BOLT: Parallelized student-specific queries
