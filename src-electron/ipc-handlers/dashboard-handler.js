...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      if (studentId) {
        const [lessonsAttended, [examStatsResult], upcomingLessonsCount] =
          await Promise.all([
            Lesson.count({
        ...
        stats.exams_taken = examStatsResult[0]?.count || 0;
        stats.success_rate = Math.round(examStatsResult[0]?.avg || 0);
