...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      // Use Promise.all to fetch admin stats in parallel and use index-friendly date comparisons
      const [
        totalStudents,
        totalInstructors,
        examsToday,
      ...
      if (instructorId) {
        ...
        // Use Promise.all to fetch instructor stats in parallel
        const [
          lessonsToday,
          assignedStudents,
          allocatedVehicle,
      ...
      if (studentId) {
        // Use Promise.all to fetch student stats in parallel
        const [
          lessonsAttended,
          examStats,
          upcomingLessons
