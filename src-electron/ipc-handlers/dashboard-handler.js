...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      const startOfToday = new Date(today.setHours(0, 0, 0, 0)).toISOString().replace('T', ' ').replace('Z', '');
      const endOfToday = new Date(today.setHours(23, 59, 59, 999)).toISOString().replace('T', ' ').replace('Z', '');

      const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1).toISOString().replace('T', ' ').replace('Z', '');

      // Optimization: Parallelize independent admin stats queries
      const [
        totalStudents,
        totalInstructors,
        examsToday,
      ...
      if (instructorId) {
        ...
        // Optimization: Parallelize independent instructor stats queries
        const [
          lessonsToday,
          assignedStudents,
          allocatedVehicle,
      ...
      if (studentId) {
        // Optimization: Parallelize independent student stats queries
        const [lessonsAttended, examStatsResult, upcomingLessons] = await Promise.all([
          Lesson.count({
