...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      // BOLT OPTIMIZATION: Use index-friendly date ranges and parallelize queries.
      ...
      const [totalStudents, totalInstructors, examsToday, revenueResult, passRateResult] = await Promise.all([
        ...
        sequelize.query(`
            ...
            `, { replacements: [firstDayOfMonth.toISOString()] }),
        sequelize.query(`
      ...
      if (instructorId) {
        // BOLT OPTIMIZATION: Parallelize independent queries for instructor dashboard.
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
      ...
      if (studentId) {
        // BOLT OPTIMIZATION: Parallelize queries for student dashboard.
        const [lessonsAttended, examStatsResult, upcomingLessonsCount] = await Promise.all([
          Lesson.count({
