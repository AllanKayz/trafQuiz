...
/**
 * Helper to get UTC date boundaries for SARGable queries.
 */
function getDateBoundaries() {
  const now = new Date();
  const startOfDay = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 0, 0, 0, 0)).toISOString().replace('T', ' ').slice(0, 23);
  const endOfDay = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 23, 59, 59, 999)).toISOString().replace('T', ' ').slice(0, 23);
  const startOfMonth = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1, 0, 0, 0, 0)).toISOString().replace('T', ' ').slice(0, 23);
  const endOfMonth = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 0, 23, 59, 59, 999)).toISOString().replace('T', ' ').slice(0, 23);
...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    const stats = {};
    const { startOfDay, endOfDay, startOfMonth, endOfMonth } = getDateBoundaries();
