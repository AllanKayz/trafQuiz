  ...
  try {
    ...
    if (role === "admin") {
      ...
      const [
        ...
        sequelize.query(
          `SELECT sum(amount) as total FROM payments WHERE type="income" AND strftime("%Y-%m", payment_date) = strftime("%Y-%m", "now")`,
        ),
        sequelize.query(
      ...
      if (studentId) {
        ...
        const [lessonsAttended, examStatsResult, upcomingLessons] =
          await Promise.all([
            Lesson.count({
