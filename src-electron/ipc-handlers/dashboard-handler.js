...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      const { start: dayStart, next: dayNext } = getDateBoundaries(new Date());
      const now = new Date();
      const { start: monthStart, next: monthNext } = getMonthBoundaries(now.getUTCFullYear(), now.getUTCMonth() + 1);
      ...
      const [
        ...
        sequelize.query(`
            ...
            `, { replacements: [monthStart, monthNext] }),
        sequelize.query(`
