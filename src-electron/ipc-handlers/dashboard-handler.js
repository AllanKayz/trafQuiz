...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    // Common date calculations for performance optimization (avoiding strftime/date() in WHERE)
    ...
    const monthStartStr = monthStart.toISOString().replace('T', ' ').replace(/\.\d+Z$/, '');

    if (role === "admin") {
      // Performance Optimization: Parallelize independent queries
      ...
      if (instructorId) {
        ...
        // Performance Optimization: Parallelize independent queries
        const [lessonsToday, assignedStudents, vehicle, upcomingLessons] = await Promise.all([
          Lesson.count({
            where: {
        ...
        stats.allocated_vehicle = vehicle;
