...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      // Optimized: Parallelize independent queries and use index-friendly date ranges.
      const now = new Date();
      ...
      if (instructorId) {
        ...
        // Optimized: Parallelize instructor-specific queries.
        const [
          lessonsToday,
          assignedStudents,
          allocatedVehicle,
      ...
      if (studentId) {
        // Optimized: Parallelize student-specific queries.
        const [
          lessonsAttended,
          examStats,
          upcomingLessons,
