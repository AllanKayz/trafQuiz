...
const { getDateBoundaries, getMonthBoundaries, toSqliteString } = require("../utils/date-utils");
...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      const { start: dayStart, end: dayEnd } = getDateBoundaries(new Date());
      const [
        totalStudents,
        totalInstructors,
        examsToday,
      ...
      if (instructorId) {
        ...
        const [lessonsToday, assignedStudents, allocatedVehicle, upcomingLessons] = await Promise.all([
          ...
          Lesson.findAll({
            where: {
              instructor_id: instructorId,
              start_time: { [Op.gte]: toSqliteString(new Date()) },
            },
            limit: 5,
