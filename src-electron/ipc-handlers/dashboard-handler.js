...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      // PERFORMANCE OPTIMIZATION: Parallelized 5 database queries and used sargable range query for exams_today.
      const [totalStudents, totalInstructors, examsToday, revenueResult, passRateResult] = await Promise.all([
        ...
        sequelize.query(
          `SELECT sum(amount) as total FROM payments WHERE type="income" AND strftime("%Y-%m", payment_date) = ?`,
      ...
      if (instructorId) {
        ...
        // PERFORMANCE OPTIMIZATION: Parallelized 4 database queries and used sargable range query for lessons_today.
      ...
      if (studentId) {
        // PERFORMANCE OPTIMIZATION: Parallelized 3 database queries.
