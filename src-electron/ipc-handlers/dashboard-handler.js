...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      // Parallelize admin dashboard queries for better performance
      const [totalStudents, totalInstructors, examsToday, revenueResult, passRateResult] = await Promise.all([
        ...
        `, { replacements: [currentMonthStart.toISOString().replace('T', ' ').replace('Z', '')], type: sequelize.QueryTypes.SELECT }),
        sequelize.query(`
