...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      const currentMonth = getMonthBoundaries(new Date().getUTCFullYear(), new Date().getUTCMonth() + 1);
      ...
      const [
        ...
        Exam.count({
          where: {
            start_time: { [Op.between]: [today.start, today.end] }
          },
        }),
        sequelize.query(`
                SELECT sum(amount) as total FROM payments
                WHERE type="income" AND payment_date BETWEEN ? AND ?
            `, { replacements: [currentMonth.start, currentMonth.end] }),
        sequelize.query(`
