...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    // Common query parameters defined once for consistency and index utilization
    ...
    if (role === "admin") {
      // Parallelize all independent admin dashboard queries
      const [
        totalStudents,
        totalInstructors,
        examsToday,
        ...
        `, { replacements: [monthStart, todayEnd] }),
        sequelize.query(`
      ...
      if (instructorId) {
        ...
        // Parallelize queries dependent on instructorId
        const [
          lessonsToday,
          assignedStudents,
          allocatedVehicle,
      ...
      if (studentId) {
        // Parallelize queries dependent on studentId
        const [
          lessonsAttended,
          examStatsResult,
          upcomingLessons
