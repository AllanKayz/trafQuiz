...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      const todayBoundaries = getDateBoundaries();
      const now = new Date();
      const monthBoundaries = getMonthBoundaries(now.getFullYear(), now.getMonth() + 1);
      ...
      const [
        ...
        Exam.count({
          where: {
            start_time: {
              [Op.gte]: todayBoundaries.start,
              [Op.lt]: todayBoundaries.next
            }
          }
        }),
        sequelize.query(`
            ...
            `, { replacements: [monthBoundaries.start, monthBoundaries.next] }),
        sequelize.query(`
