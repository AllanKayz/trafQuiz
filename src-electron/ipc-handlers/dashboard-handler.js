    ...
    if (role === "admin") {
      ...
      const [totalStudents, totalInstructors, examsToday, revenueResult, passRateResult] = await Promise.all([
        ...
        sequelize.query(
          ...
          { replacements: [currentMonthStart.toISOString().split("T")[0]] }
        ),
        sequelize.query(
      ...
      if (studentId) {
        const [lessonsAttended, examStats, upcomingLessons] = await Promise.all([
          Lesson.count({ where: { student_id: studentId, status: "completed" } }),
          sequelize.query(
            `SELECT count(*) as count, AVG(score) as avg FROM student_exams WHERE student_id = ?`,
            { replacements: [studentId] }
          ),
          Lesson.count({ where: { student_id: studentId, status: "upcoming" } }),
        ]);
