...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      // PERFORMANCE: Parallelize independent database queries for Admin dashboard
      const [
        totalStudents,
        totalInstructors,
        examsToday,
        ...
        Instructor.count(),
        // PERFORMANCE: Use Op.between instead of sequelize.fn('date') to allow index usage on start_time
        Exam.count({
          where: {
      ...
      if (instructorId) {
        ...
        // PERFORMANCE: Parallelize independent database queries for Instructor dashboard
        const [
          lessonsToday,
          assignedStudents,
          allocatedVehicle,
      ...
      if (studentId) {
        // PERFORMANCE: Parallelize independent database queries for Student dashboard
        const [
          lessonsAttended,
          examStats,
