...
ipcMain.handle("get-dashboard-stats", async (event, params) => {
  try {
    ...
    if (role === "admin") {
      ...
      const today = getDateBoundaries(new Date());
      const now = new Date();
      const thisMonth = getMonthBoundaries(now.getUTCFullYear(), now.getUTCMonth() + 1);
      ...
      const [
        ...
        sequelize.query(`
            ...
            `, { replacements: [thisMonth.start, thisMonth.next] }),
        sequelize.query(`
