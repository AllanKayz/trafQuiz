  ...
  try {
    ...
    if (role === "admin") {
      ...
      const [
        ...
        Instructor.count(),
        // Use index-friendly range query instead of fn('date')
        Exam.count({
          where: {
            start_time: { [Op.between]: [todayStart, todayEnd] },
          },
        }),
        // Use index-friendly range query instead of strftime
        sequelize.query(
          `SELECT sum(amount) as total FROM payments
           WHERE type="income" AND payment_date >= ? AND payment_date < ?`,
          { replacements: [firstOfMonth.toISOString(), firstOfNextMonth.toISOString()] }
        ),
        sequelize.query(`
      ...
      if (instructorId) {
        ...
        const [lessonsToday, assignedStudents, allocatedVehicle, upcomingLessons] =
          await Promise.all([
