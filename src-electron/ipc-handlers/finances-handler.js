...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Performance Optimization: Fetch totals in parallel
    const [totalRevenue, totalExpenses] = await Promise.all([
      Payment.sum("amount", { where: { type: "income" } }).then(v => Number(v) || 0),
      Payment.sum("amount", { where: { type: "expense" } }).then(v => Number(v) || 0)
    ]);
    ...
    // Performance Optimization: Replace N+1 queries in a loop with a single aggregate query
    ...
    const resultsMap = {};
    results.forEach(r => {
      resultsMap[`${r.monthYear}_${r.type}`] = Number(r.total) || 0;
    });
    ...
    for (const m of months) {
      ...
      chartRevenue.push(resultsMap[`${monthKey}_income`] || 0);
      chartExpenses.push(resultsMap[`${monthKey}_expense`] || 0);
    }
