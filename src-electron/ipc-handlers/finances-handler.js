...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize total revenue and expense fetching
    ...
    // Calculate dynamic chart data for the last 6 months (ordered oldest to newest)
    const months = [];
    const monthNames = [
      "Jan",
      "Feb",
    ...
    // Optimize: Fetch all chart data in a single aggregate query instead of 12 separate queries
    // months[0] is the oldest month in our range (5 months ago)
    ...
    // Map aggregate results back to the ordered months array using a hash map for O(1) lookup
    const statsMap = results.reduce((acc, row) => {
      acc[row.monthKey] = row;
      return acc;
    }, {});

    const chartRevenue = labels.map(label => Number(statsMap[label]?.revenue) || 0);
    const chartExpenses = labels.map(label => Number(statsMap[label]?.expenses) || 0);
