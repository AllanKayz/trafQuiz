...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // PERFORMANCE: Parallelize total sums fetching
    ...
    // PERFORMANCE: Use a single aggregate query instead of a loop of 12 queries
    ...
    // Map results to the 6-month array ensuring every month has a value (even if 0)
    const chartRevenue = labels.map(label => {
      const found = chartResults.find(r => r.month_key === label);
      return Number(found?.revenue) || 0;
    });

    const chartExpenses = labels.map(label => {
      const found = chartResults.find(r => r.month_key === label);
      return Number(found?.expenses) || 0;
    });
