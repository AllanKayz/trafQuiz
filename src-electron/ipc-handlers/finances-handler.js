...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize total aggregation queries
    ...
    // Optimize: Fetch all chart data in a single aggregate query instead of 12 separate ones
    const monthYearConditions = months.map(m =>
      `(strftime('%m', payment_date) = '${m.month.toString().padStart(2, "0")}' AND strftime('%Y', payment_date) = '${m.year}')`
    ).join(' OR ');
    ...
    // Map results back to the ordered month array
    const resultsMap = results.reduce((acc, curr) => {
      acc[curr.label] = curr;
      return acc;
    }, {});

    const chartRevenue = labels.map(label => Number(resultsMap[label]?.revenue) || 0);
    const chartExpenses = labels.map(label => Number(resultsMap[label]?.expenses) || 0);
