...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Calculate dynamic labels for the last 6 months
    const months = [];
    ...
    const labels = months.map((m) => `${m.year}-${m.monthStr}`);

    // Optimization: Parallelize top-level sums and fetch all chart data in a single aggregate query
    // This replaces 12 sequential queries with 1 efficient GROUP BY query.
    const [totalRevenue, totalExpenses, chartResults] = await Promise.all([
      ...
      sequelize.query(
        ...
        `,
        {
          replacements: [`${months[0].year}-${months[0].monthStr}-01`],
          type: sequelize.QueryTypes.SELECT
        }
      )
    ]);

    // Map chart results to the 6-month slots
    const chartRevenue = new Array(6).fill(0);
    const chartExpenses = new Array(6).fill(0);

    chartResults.forEach(row => {
      const index = labels.indexOf(row.monthYear);
      if (index !== -1) {
        chartRevenue[index] = Number(row.revenue) || 0;
        chartExpenses[index] = Number(row.expenses) || 0;
      }
    });
