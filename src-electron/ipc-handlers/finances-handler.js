...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // 1. Parallelize total revenue and expense fetching
    ...
    // 2. Optimized 6-month chart data fetching (Single aggregate query)
    const months = [];
    ...
    // Map results to the months array to handle gaps (months with no data)
    const resultsMap = {};
    ...
    for (const m of months) {
      const data = resultsMap[m.key];
      chartRevenue.push(data ? Number(data.revenue) || 0 : 0);
      chartExpenses.push(data ? Number(data.expenses) || 0 : 0);
    }
