...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize total calculations
    ...
    // Optimize: Single aggregate query for all 6 months of chart data
    ...
    // Map results to the 6-month array (ensuring zeros for months with no data)
    const resultsMap = {};
    chartResults.forEach((r) => {
      resultsMap[r.month_key] = r;
    });

    const chartRevenue = months.map((m) => resultsMap[m.key]?.revenue || 0);
    const chartExpenses = months.map((m) => resultsMap[m.key]?.expenses || 0);
