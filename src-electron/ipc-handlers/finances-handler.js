...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize initial sum queries and last 6 months calculation
    ...
    // Optimize: Use a single aggregate query instead of a loop to reduce DB round-trips
    ...
    // Map aggregate results back to the expected array format for the chart
    const resultsMap = {};
    chartResults.forEach(r => {
      resultsMap[r.monthStr] = { revenue: r.revenue, expenses: r.expenses };
    });

    const chartRevenue = labels.map(l => resultsMap[l]?.revenue || 0);
    const chartExpenses = labels.map(l => resultsMap[l]?.expenses || 0);
