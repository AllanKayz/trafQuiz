...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    const minMonth = months[0].key;
    const maxMonth = months[5].key;

    // Use a single aggregate query to fetch all monthly totals for the 6-month period
    const [monthlyTotals] = await sequelize.query(`
      ...
      GROUP BY month_key
    `, { replacements: [minMonth, maxMonth] });

    const totalsMap = monthlyTotals.reduce((acc, curr) => {
      acc[curr.month_key] = curr;
      return acc;
    }, {});

    const chartRevenue = months.map(m => Number(totalsMap[m.key]?.revenue) || 0);
    const chartExpenses = months.map(m => Number(totalsMap[m.key]?.expenses) || 0);
