...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Optimize: Use a single aggregate query instead of looping through months
    ...
    const resultsMap = monthlyResults.reduce((acc, row) => {
      acc[row.monthLabel] = row;
      return acc;
    }, {});

    const chartRevenue = labels.map((l) => Number(resultsMap[l]?.revenue) || 0);
    const chartExpenses = labels.map(
      (l) => Number(resultsMap[l]?.expenses) || 0,
    );
