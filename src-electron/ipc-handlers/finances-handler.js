...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Current date for 6-month calculation
    const now = new Date();
    for (let i = 5; i >= 0; i--) {
    ...
    // Optimized: Fetch all 6 months of data in a single aggregate query
    ...
    // Map aggregate data back to our labels to ensure even empty months are included
    const monthlyMap = monthlyData.reduce((acc, row) => {
      acc[row.label] = row;
      return acc;
    }, {});
    ...
    for (const m of months) {
      const row = monthlyMap[m.label] || { revenue: 0, expenses: 0 };
      chartRevenue.push(Number(row.revenue) || 0);
      chartExpenses.push(Number(row.expenses) || 0);
      labels.push(m.label);
    }
