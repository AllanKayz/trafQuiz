...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Optimize: Use a single aggregate query for the 6-month chart
    // instead of 12 sequential queries.
    ...
    const statsMap = chartResults.reduce((acc, row) => {
      acc[row.label] = row;
      return acc;
    }, {});

    const chartRevenue = labels.map((l) => statsMap[l]?.revenue || 0);
    const chartExpenses = labels.map((l) => statsMap[l]?.expenses || 0);
