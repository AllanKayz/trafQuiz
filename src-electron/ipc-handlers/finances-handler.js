...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Optimize: Parallelize total sums and a single aggregate query for chart data
    const [totalRevenueRaw, totalExpensesRaw, [monthlyStats]] = await Promise.all([
    ...
    monthlyStats.forEach(stat => statsMap.set(stat.month_key, stat));
    ...
    for (const label of labels) {
      ...
      chartRevenue.push(Number(stat?.revenue) || 0);
      chartExpenses.push(Number(stat?.expenses) || 0);
    }
