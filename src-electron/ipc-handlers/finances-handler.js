...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize core revenue/expense queries
    ...
    // Optimized: Replace 12-query loop with a single aggregate query using date range
    ...
    // Map aggregate results back to the labels array to ensure correct ordering and fill gaps
    const statsMap = {};
    aggregateStats.forEach(row => {
      statsMap[row.month_key] = {
        revenue: Number(row.revenue) || 0,
        expenses: Number(row.expenses) || 0
      };
    });

    const chartRevenue = labels.map(label => statsMap[label]?.revenue || 0);
    const chartExpenses = labels.map(label => statsMap[label]?.expenses || 0);
