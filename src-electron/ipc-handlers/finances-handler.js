...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Optimized: Replace 6-month sequential query loop with a single aggregate SQL query
    // This reduces the number of queries from 12 to 1.
    const startOfMonth = labels[0] + "-01";
    ...
    const chartRevenue = labels.map((label) => {
      const match = chartResults.find(
        (r) => r.month_key === label && r.type === "income",
      );
    ...
    const chartExpenses = labels.map((label) => {
      const match = chartResults.find(
        (r) => r.month_key === label && r.type === "expense",
      );
      return match ? Number(match.total) : 0;
    });
