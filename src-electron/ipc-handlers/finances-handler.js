...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize initial total calculations
    ...
    // Optimized: Replace the loop of queries with a single aggregate SQL query
    // Use the earliest month as the start of the range to leverage the payment_date index
    const firstMonth = months[0];
    const startDate = `${firstMonth.year}-${firstMonth.month.toString().padStart(2, '0')}-01 00:00:00`;

    const [aggregateResults] = await sequelize.query(
      `
      SELECT
        strftime('%Y-%m', payment_date) as monthStr,
    ...
    // Map aggregate results back to the requested months
    const resultMap = {};
    aggregateResults.forEach(r => {
      resultMap[r.monthStr] = r;
    });

    const chartRevenue = labels.map(label => resultMap[label]?.revenue || 0);
    const chartExpenses = labels.map(label => resultMap[label]?.expenses || 0);
