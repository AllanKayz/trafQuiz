...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Parallelize the initial aggregate sum queries
    ...
    // Parallelize all month-by-month queries
    const chartDataQueries = months.map(async (m) => {
    ...
    const chartResults = await Promise.all(chartDataQueries);
