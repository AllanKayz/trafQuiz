...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Optimize: Parallelize independent aggregate queries
    ...
    // Optimize: Single aggregate query for all months instead of loop with 12 queries
    ...
    const chartRevenue = months.map(m => Number(statsMap[m.key]?.revenue) || 0);
    const chartExpenses = months.map(m => Number(statsMap[m.key]?.expenses) || 0);
