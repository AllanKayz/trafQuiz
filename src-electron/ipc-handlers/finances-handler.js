...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // 1. Prepare date ranges for the last 6 months
    const months = [];
    ...
    const startDate = `${firstMonth.yearStr}-${firstMonth.monthStr}-01`;

    // 2. Fetch all data in parallel
    const [totalRevenue, totalExpenses, monthlyStats] = await Promise.all([
    ...
    // 3. Map monthly stats to chart arrays, ensuring all 6 months are present
    const chartRevenue = labels.map(label => {
      const stat = monthlyStats.find(s => s.monthYear === label);
      return Number(stat?.revenue) || 0;
    });

    const chartExpenses = labels.map(label => {
      const stat = monthlyStats.find(s => s.monthYear === label);
      return Number(stat?.expenses) || 0;
    });

    const finalRevenue = totalRevenue || 0;
