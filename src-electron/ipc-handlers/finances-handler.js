...
ipcMain.handle("get-financial-stats", async () => {
  try {
    ...
    // Fetch all 6 months of data in a single aggregate query
    const [monthlyData] = await sequelize.query(
      `
      SELECT
        strftime('%Y-%m', payment_date) as month_label,
    ...
    const dataMap = monthlyData.reduce((acc, curr) => {
      acc[curr.month_label] = curr;
      return acc;
    }, {});

    const chartRevenue = labels.map(label => Number(dataMap[label]?.revenue) || 0);
    const chartExpenses = labels.map(label => Number(dataMap[label]?.expenses) || 0);
