...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    // 1. student_exams - for progress and pass rates
    ...
    // 2. payments - for financial stats and chart generation
    await queryInterface.addIndex('payments', ['payment_date']);
    await queryInterface.addIndex('payments', ['type']);

    // 3. lessons - for filtering by status (upcoming/completed)
    await queryInterface.addIndex('lessons', ['status']);

    // 4. exams - for filtering by start_time (exams today)
