...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    // SQLite can use this for WHERE type = 'income' AND payment_date >= ...
    ...
    // 2. Student Exams - optimize progress tracking
    await queryInterface.addIndex('student_exams', ['student_id', 'completed_at'], {
      name: 'student_exams_student_date_idx'
    });

    // 3. Exams - optimize daily count on dashboard
    await queryInterface.addIndex('exams', ['start_time'], {
      name: 'exams_start_time_idx'
    });

    // 4. Lessons - optimize instructor dashboard
    // Replacing single column index with a composite one that handles range on start_time
