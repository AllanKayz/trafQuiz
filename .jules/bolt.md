...
## 2026-04-27 - [SARGable Queries & Composite Indexes]
**Learning:** SQLite cannot use indexes on columns wrapped in functions like `strftime` or `fn('date')` in `WHERE` clauses. Refactoring these into `BETWEEN` range queries with composite indexes on frequently filtered columns (e.g., `payment_date, type, status`) yields dramatic latency reductions (~40-60%).
**Action:** Always prefer `Op.between` for date ranges over `strftime` comparisons. Add composite indexes that match the query's filtering and sorting requirements.
