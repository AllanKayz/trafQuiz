...
## 2026-05-10 - [SARGability & SQLite String Functions]
**Learning:** Using SQLite functions like `strftime` or `date()` in `WHERE` clauses prevents index usage (non-SARGable). Refactoring these to range queries (`BETWEEN`) with pre-calculated boundaries in JS enables 'SEARCH' plans. Additionally, `SUBSTR(col, 1, 7)` is more efficient than `strftime('%Y-%m', col)` for monthly grouping in SQLite.
**Action:** Always refactor date-based `WHERE` clauses to range queries. Use `SUBSTR` for grouping by month/day to minimize function overhead in large aggregations.
