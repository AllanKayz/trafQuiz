...
**Learning:** SQLite cannot use indexes for queries where the column is wrapped in a function (e.g., `date(start_time)` or `strftime('%Y-%m', payment_date)`). Refactoring these into range queries using `Op.between` with pre-calculated UTC boundaries makes the queries SARGable (Search ARGumentable). Additionally, using `SUBSTR(column, 1, 7)` is a more efficient way to group by month in SQLite than `strftime` when the filter is already SARGable.
**Action:** Always check `EXPLAIN QUERY PLAN` to ensure indexes are being used. Avoid functions on indexed columns in `WHERE` clauses; pre-calculate range boundaries in the application layer instead.
