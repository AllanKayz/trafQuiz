...
## 2026-05-10 - [SARGable Queries & Date Consistency]
**Learning:** Using SQL functions like `date()` or `strftime()` on columns in WHERE clauses prevents SQLite from using indexes (non-SARGable). Additionally, mixing local `Date` objects with UTC database storage leads to timezone-related logic bugs.
**Action:** Replace date functions with `Op.between` range queries. Always use `Date.UTC` to calculate boundaries for "today" or "this month" and format them as `YYYY-MM-DD HH:mm:ss` strings to match SQLite's native storage format for reliable indexed searches.
