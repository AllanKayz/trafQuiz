...
**Learning:** Using SQLite functions (like `strftime` or `date()`) in WHERE clauses prevents the query optimizer from using indexes (non-SARGable). Refactoring these to range comparisons (`BETWEEN` or `>=`) combined with composite indexes (e.g., `(instructor_id, start_time)`) dramatically reduces query latency and ensures scalability.
**Action:** Always refactor function-based WHERE filters into range queries. Use `EXPLAIN QUERY PLAN` to verify that the database performs an indexed `SEARCH` instead of a full `SCAN`.
