...
class VehiclesController {
    public function getVehicles() {
        ...
        if ($id) {
            ...
            if (!$v) { http_response_code(404); echo json_encode(['message' => 'Vehicle not found']); return; }
            echo json_encode($v); return;
        }
    ...
    public function add() {
        ...
        $created = VehicleModel::add($input);
    ...
    public function update() {
        ...
        $input = json_decode(file_get_contents('php://input'), true) ?: $_POST;
        $id = $input['id'] ?? null;
        if (!$id) { http_response_code(400); echo json_encode(['message' => 'Missing id']); return; }
        unset($input['id']);
        $ok = VehicleModel::update($id, $input);
        if (!$ok) { http_response_code(404); echo json_encode(['message' => 'Vehicle not found']); return; }
