...
import { ApiResponse, Question, TraffiquizService } from '../../traffiquiz.service';
...
export class QuestionsComponent {
  ...
  private async deleteQuestion(id: number) {
    if (confirm('Are you sure you want to delete this question?')) {
      try {
        await this.trafQuizService.deleteQuestion(id);
  ...
  private async saveQuestion(data: any, id?: number) {
    const questionData = {
      id: id || 0,
    ...
    try {
      if (id) {
        await this.trafQuizService.updateQuestion(questionData);
      } else {
        await this.trafQuizService.addQuestion(questionData);
      }
      this.snackBar.open(`Question ${id ? 'updated' : 'added'} successfully`, 'Close', { duration: 3000 });
      this.trafQuizService.fetchQuestions();
    } catch (error) {
      this.snackBar.open('Error saving question', 'Close', { duration: 3000 });
    }
  }
